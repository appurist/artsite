...
async function performRestore() {
  try {
    ...
    // Wait a moment then close modal and show results
    setTimeout(() => {
      closeModal();
...
function updateRestoreProgress(message, progress) {
  // Find or create progress element
  let progressElement = document.getElementById('restore-progress');
  if (!progressElement) {
    const modalActions = document.querySelector('.modal-actions');
    if (modalActions) {
      progressElement = document.createElement('div');
      progressElement.id = 'restore-progress';
      progressElement.className = 'restore-progress';
      modalActions.parentNode.insertBefore(progressElement, modalActions);
    }
  }
  ...
  if (progressElement) {
    if (progress >= 0) {
      ...
      // Error state
      progressElement.innerHTML = `
        <div class="progress-message error">${message}</div>
      `;
    }
  }
}
